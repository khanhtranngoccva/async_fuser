...
pub use crate::ll::flags::access_flags::AccessFlags;
pub use crate::ll::flags::bsd_file_flags::BsdFileFlags;
...
pub use crate::ll::flags::open_flags::OpenAccMode;
pub use crate::ll::flags::open_flags::OpenFlags;
pub use crate::ll::flags::poll_flags::PollFlags;
pub use crate::ll::flags::rename_flags::RenameFlags;
...
pub use crate::session::RuntimeStrategy;
...
impl KernelConfig {
    fn new(capabilities: InitFlags, max_readahead: u32, kernel_abi: Version) -> Self {
        Self {
            capabilities,
            requested: init_flags::default_init_flags(capabilities),
            max_readahead,
...
pub trait Filesystem: Send + Sync + 'static {
    ...
    async fn lookup(&self, _req: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEntry) {
        ...
        reply.error(Errno::ENOSYS).await;
    }
    ...
    async fn batch_forget(&self, req: &Request, nodes: &[ForgetOne]) {
        for node in nodes {
    ...
    /// Read symbolic link.
    async fn readlink(&self, _req: &Request, ino: INodeNo, reply: ReplyData) {
    ...
    /// Remove a file.
    async fn unlink(&self, _req: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEmpty) {
        warn!("[Not Implemented] unlink(parent: {parent:#x?}, name: {name:?})",);
        reply.error(Errno::ENOSYS).await;
    }

    /// Remove a directory.
    async fn rmdir(&self, _req: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEmpty) {
    ...
    async fn open(&self, _req: &Request, _ino: INodeNo, _flags: OpenFlags, reply: ReplyOpen) {
        reply.opened(FileHandle(0), FopenFlags::empty()).await;
    }
    ...
    async fn opendir(&self, _req: &Request, _ino: INodeNo, _flags: OpenFlags, reply: ReplyOpen) {
        reply.opened(FileHandle(0), FopenFlags::empty()).await;
    }
    ...
    async fn statfs(&self, _req: &Request, _ino: INodeNo, reply: ReplyStatfs) {
    ...
    /// If `size` is not 0, and the value fits, send it with `reply.data()`, or
    /// `reply.error(ERANGE)` if it doesn't.
    async fn listxattr(&self, _req: &Request, ino: INodeNo, size: u32, reply: ReplyXattr) {
        warn!("[Not Implemented] listxattr(ino: {ino:#x?}, size: {size})");
        reply.error(Errno::ENOSYS).await;
    }

    /// Remove an extended attribute.
    async fn removexattr(&self, _req: &Request, ino: INodeNo, name: &OsStr, reply: ReplyEmpty) {
    ...
    async fn access(&self, _req: &Request, ino: INodeNo, mask: AccessFlags, reply: ReplyEmpty) {
    ...
    async fn bmap(&self, _req: &Request, ino: INodeNo, blocksize: u32, idx: u64, reply: ReplyBmap) {
    ...
    #[cfg(target_os = "macos")]
    async fn setvolname(&self, _req: &Request, name: &OsStr, reply: ReplyEmpty) {
    ...
    #[cfg(target_os = "macos")]
    async fn getxtimes(&self, _req: &Request, ino: INodeNo, reply: ReplyXTimes) {
