...
impl<'a> Notification<'a> {
    pub(crate) async fn with_iovec<F, T>(
        &self,
        code: abi::fuse_notify_code,
        f: F,
    ) -> Result<T, TryFromIntError>
    where
        F: AsyncFnOnce(&[IoSlice<'_>]) -> T,
    {
...
mod test {
    ...
    async fn store() {
        let n = Notification::new_store(INodeNo(0x42), 50, &[0xde, 0xad, 0xbe, 0xef])
            .unwrap()
            .with_iovec(abi::fuse_notify_code::FUSE_NOTIFY_STORE, async |iov| {
                ioslice_to_vec(iov)
            })
            .await
            .unwrap();
    ...
    async fn delete() {
        let n = Notification::new_inval_entry(INodeNo(0x42), OsStr::new("abc"))
            .unwrap()
            .with_iovec(abi::fuse_notify_code::FUSE_NOTIFY_DELETE, async |iov| {
                ioslice_to_vec(iov)
            })
            .await
            .unwrap();
    ...
    async fn poll() {
        let n = Notification::new_poll(PollHandle(0x4321))
            .with_iovec(abi::fuse_notify_code::FUSE_POLL, async |iov| {
                ioslice_to_vec(iov)
            })
            .await
            .unwrap();
