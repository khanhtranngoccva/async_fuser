...
pub(crate) trait Response {
    ...
    async fn with_iovec<F: AsyncFnOnce(&[IoSlice<'_>]) -> T, T>(
        &self,
        unique: RequestId,
        f: F,
    ) -> T {
        ...
        IosliceConcat::with_ioslice(&v, f).await
    }
}
...
mod test {
    ...
    async fn reply_empty() {
        let r = ResponseEmpty;
        assert_eq!(
            r.with_iovec(RequestId(0xdeadbeef), async |slices| ioslice_to_vec(slices))
                .await,
            vec![
