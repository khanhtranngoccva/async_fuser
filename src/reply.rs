...
pub(crate) enum ReplySender {
    ...
    Assert(AssertSender),
    #[cfg(test)]
    Sync(tokio::sync::mpsc::Sender<()>),
}

impl ReplySender {
    /// Send data.
    pub(crate) async fn send(&self, data: &[IoSlice<'_>]) -> std::io::Result<()> {
        match self {
            ReplySender::Channel(sender) => sender.send(data).await,
            #[cfg(test)]
            ReplySender::Assert(sender) => sender.send(data).await,
            #[cfg(test)]
    ...
    /// Open a backing file
    pub(crate) async fn open_backing(&self, fd: BorrowedFd<'_>) -> std::io::Result<BackingId> {
        match self {
            ReplySender::Channel(sender) => sender.open_backing(fd).await,
            #[cfg(test)]
...
impl ReplyRaw {
    ...
    pub(crate) async fn send_ll(mut self, response: &impl Response) {
        self.send_ll_mut(response).await;
    }

    /// Reply to a request with the given error code
    pub(crate) async fn error(self, err: ll::Errno) {
        self.send_ll(&ll::ResponseErrno(err)).await;
    }
}
...
impl ReplyEmpty {
    ...
    pub async fn ok(self) {
        self.reply.send_ll(&ll::ResponseEmpty).await;
    }
...
impl ReplyData {
    ...
    pub async fn data(self, data: &[u8]) {
        self.reply.send_ll(&ll::ResponseSlice(data)).await;
    }
...
impl ReplyEntry {
    ...
    pub async fn entry(self, ttl: &Duration, attr: &FileAttr, generation: Generation) {
        self.reply
            .send_ll(&ll::ResponseStruct::new_entry(
                attr.ino,
                generation,
                &attr.into(),
                *ttl,
...
impl ReplyOpen {
    ...
    pub async fn opened(self, fh: ll::FileHandle, flags: FopenFlags) {
    ...
    pub async fn opened_passthrough(
        self,
        fh: ll::FileHandle,
        flags: FopenFlags,
        ...
        self.reply
            .send_ll(&ll::ResponseStruct::new_open(
                fh,
                flags,
...
impl ReplyWrite {
    ...
    pub async fn written(self, size: u32) {
        self.reply
            .send_ll(&ll::ResponseStruct::new_write(size))
            .await;
    }
...
impl ReplyStatfs {
    ...
    pub async fn statfs(
        ...
        self.reply
            .send_ll(&ll::ResponseStruct::new_statfs(
...
impl ReplyCreate {
    ...
    pub async fn created(
        ...
        assert!(!flags.contains(FopenFlags::FOPEN_PASSTHROUGH));
        self.reply
            .send_ll(&ll::ResponseStruct::new_create(
                ttl,
                &attr.into(),
                generation,
                fh,
    ...
    /// Note that you must reuse the given `BackingId` for all future [`ReplyOpen::opened_passthrough()`]
    /// invocations as long as the passed file handle stays open!
    pub async fn created_passthrough(
        self,
        ttl: &Duration,
        attr: &FileAttr,
        generation: Generation,
...
impl ReplyLock {
    ...
    pub async fn locked(self, start: u64, end: u64, typ: i32, pid: u32) {
        self.reply
            .send_ll(&ll::ResponseStruct::new_lock(&ll::Lock {
                range: (start, end),
                typ,
                pid,
            }))
...
impl ReplyBmap {
    ...
    pub async fn bmap(self, block: u64) {
        self.reply
            .send_ll(&ll::ResponseStruct::new_bmap(block))
            .await;
    }
...
impl ReplyPoll {
    ...
    pub async fn poll(self, revents: PollEvents) {
        self.reply
            .send_ll(&ll::ResponseStruct::new_poll(revents))
            .await;
    }
...
impl ReplyDirectory {
    ...
    pub async fn ok(self) {
        ...
        self.reply.send_ll(&response).await;
    }
...
impl ReplyLseek {
    ...
    pub async fn offset(self, offset: i64) {
        self.reply
            .send_ll(&ll::ResponseStruct::new_lseek(offset))
            .await;
    }
...
mod test {
    ...
    async fn reply_entry() {
        ...
        reply.entry(&ttl, &attr, ll::Generation(0xaa)).await;
    }
